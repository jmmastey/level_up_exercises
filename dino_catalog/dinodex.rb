...
def main
	...
	while(1)
		...
		when '5'
			...
			puts ((get_dino_by_name name)).join(', ')
		when 'c'
			@chain &= @last_result
			puts 'chain updated'
		when 'r'
...
def get_default_dino_by_name name
	return filter_dinos_by_and_return_by(@default_dinos, :name, name, :name)
end

def get_african_dino_by_name name
	return filter_dinos_by_and_return_by(@african_dinos, :genus, name, :genus)
end
...
def get_default_dinos_by_size size
	return filter_dinos_by_and_return_by(@default_dinos, :weight_in_lbs, size, :name)
end

def get_african_dinos_by_size size
	return filter_dinos_by_and_return_by(@african_dinos, :weight, size, :genus)
end
...
def get_default_dinos_bipeds
	return filter_dinos_by_and_return_by(@default_dinos, :walking, 'biped', :name)
end

def get_african_dinos_bipeds
	return filter_dinos_by_and_return_by(@african_dinos, :walking, 'biped', :genus)
end
...
def get_default_dinos_carnivores
	return (filter_dinos_by_and_return_by @default_dinos, :diet, 'carnivore', :name)
...
def get_african_dinos_carnivores
	return (filter_dinos_by_and_return_by @african_dinos, :carnivore, 'yes', :genus)
end
...
def get_default_dinos_by_period period
	return (filter_dinos_by_and_return_by @default_dinos, :period, period, :name)
end

def get_african_dinos_by_period period
	return (filter_dinos_by_and_return_by @african_dinos, :period, period, :genus)
end
...
def filter_dinos_by_and_return_by dinos, filter_by, filter_value, key
	...
	dinos.each do |dino|
		...
		elsif filter_by == :name || filter_by == :genus
			filter_by_name dino, filter_by, filter_value, results, key
		elsif filter_by == :weight_in_lbs || filter_by == :weight
...
def filter_by_period dino, filter_by, filter_value, results, key
	unless dino[filter_by].nil?
		if dino[filter_by].downcase.include? filter_value.downcase
			results << dino[key]
		end
	end
end
...
def filter_by_name dino, filter_by, filter_value, results, key
	unless dino[filter_by].nil?
		if dino[filter_by].downcase == filter_value.downcase
			results << dino[key]
			puts ''
...
def filter_by_weight dino, filter_by, filter_value, results, key
	unless dino[filter_by].nil?
		case filter_value
		when 'b'
			if dino[filter_by].downcase.to_i > 4000
				results << dino[key]
			end
		when 's'
			if dino[filter_by].downcase.to_i < 4001
				results << dino[key]
			end
		end
	end
