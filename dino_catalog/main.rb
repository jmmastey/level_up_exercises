...
puts dinodex.filter(:diet, "Herbivore").filter(:walking, "Quadruped").filter(:continent, "Africa").to_s

#JSON examples
puts dinodex.filter(:name, "Suchomimus").to_json
puts dinodex.filter(:diet, "Herbivore").to_json
puts dinodex.filter(:weight, "small").filter(:continent, "North America").to_json
