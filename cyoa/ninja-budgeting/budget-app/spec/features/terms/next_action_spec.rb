...
describe 'next term page actions' do
  ...
  context 'user has no next term' do
    before :each do
      visit '/next'
    end

    it 'allows you to create a new next term if it doesn\'t exist' do
      expect(page).to have_content('Create one now?')
      find('#create-new-next-term').click
      page.find('#newNextTermModal').visible?
      within(:css, '#newNextTermModal') do
  ...
  context 'user has next term' do
    ...
    it 'allows user to add money to budgeted category', js: true do
      ...
      expect(page).to have_content("Reserved: #{Date::MONTHNAMES[(Time.now + 1.month).month]} #{(Time.now + 1.month).year}")
    ...
    it 'allows user to add transaction', js: true do
      ...
      expect(page).to have_content(@fund.name)
      find('#funds-table .expandable-category-row').click
      expect(page).to have_content("Pending: #{Date::MONTHNAMES[(Time.now + 1.month).month]} #{(Time.now + 1.month).year}")
      expect(page).to have_content('$25.00')
    end
    ...
    it 'can change paid status and update fund', js: true do
      ...
      visit '/funds'
      find('#funds-table .expandable-category-row').click
      expect(page).to_not have_content("Pending: #{Date::MONTHNAMES[(Time.now + 1.month).month]} #{(Time.now + 1.month).year}")
