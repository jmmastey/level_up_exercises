...
describe Datacalculation do
  let(:data) { { 'A' => { total: 500.0, conversion: 340.0 },
  ...
  context 'when data calculator is created' do
    ...
    it 'outputs the lower and higher level of percentage' do
      expect(data_calc.conversion_percentage('A').first).to eq(63.91)
      expect(data_calc.conversion_percentage('A').last).to eq(72.09)
    end
