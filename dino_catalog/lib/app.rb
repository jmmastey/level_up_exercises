...
class App
  ...
  def filter_results(catalog, filters)
    filtered_dinosaur_listings = catalog
    filters.each do |search_type, criteria|
      filter_function = "filter_#{search_type}".to_sym
      filtered_dinosaur_listings = filtered_dinosaur_listings.send(filter_function, criteria)
    end
    filtered_dinosaur_listings.flatten
  end
