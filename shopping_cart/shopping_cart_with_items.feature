...
Scenario: Normal line contents
  ...
  And the name line shows item name
  And the price line shows item unit price $5
  And the quantity line shows quantity 1 of item
  And the net price line shows item net price $5
  And the description line shows item description
  And the stock number line shows item stock number
  And the item page line show a link to item page
...
Scenario: Line for multiples of same item
  ...
  When I visit the cart item page
  Then I see a line for 1 item
  And the item price line shows item unit price $5
  And the quantity line shows quantity 5 of item
  And the net price line shows item net price $25
...
Scenario: Coupon line
  ...
  And the coupon code line shows the coupon code
  And the coupon promotion line shows the coupon promotion name
  And the net price line shows item net price -$5
...
Scenario: Line for each coupon in effect
  ...
  And each item price line has item price -$2
...
Scenario: Sales Tax line
  ...
  And the item price line has item price $0.05
...
Scenario: Lines for out-of-stock item
  ...
  And the warning line shows an out-of-stock warning
...
Scenario: Price change for item already in cart
  ...
  When I visit the cart item page
  Then the warning line shows a price-change warning
