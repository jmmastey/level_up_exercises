...
describe DataCalculation do
  ...
  context 'when data calculator is created' do
    ...
    it 'with invalid group name it will give error' do
      expect { data_calc.total_size('C') }.to raise_error(ArgumentError)
      expect { data_calc.number_of_conversion('C') }.to raise_error(ArgumentError)
      expect { data_calc.conversion_rate('C') }.to raise_error(ArgumentError)
    end
