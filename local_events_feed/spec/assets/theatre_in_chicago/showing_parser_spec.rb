...
RSpec.describe TheatreInChicago::ShowingParser, :type => :asset do
  ...
  let(:same_year_parser) { TheatreInChicago::DateListShowingParser.new(same_year_body, pseudo_today) }

  let(:diff_year_body) { File.open("test/fixtures/theatre_in_chicago_test_showings_diff_year.html").read }
  let(:diff_year_parser) { TheatreInChicago::DateListShowingParser.new(diff_year_body, pseudo_today) }

  let(:thru_date_body) { File.open("test/fixtures/theatre_in_chicago_test_showings_thru_date.html").read }
  let(:thru_date_parser) { TheatreInChicago::DowsShowingParser.new(thru_date_body, pseudo_today) }
  ...
  let(:test_body) { File.open("test/fixtures/theatre_in_chicago_test_showings_test.html").read }
  let(:test_parser) { TheatreInChicago::DateListShowingParser.new(test_body, pseudo_today) }
