...
RSpec.describe "Factory Girl" do
  valid_factories = FactoryGirl.factories.reject do |factory|
    factory.name =~ /^invalid_/
  end
  valid_factories.map(&:name).each do |factory_name|
    describe "#{factory_name} factory" do
      ...
      it "is valid" do
        ...
        if factory.respond_to?(:valid?)
          ...
          # for earlier versions, you have to call #valid? before
          # calling the matcher, otherwise the errors will be empty
          expect(factory)
          .to be_valid,
