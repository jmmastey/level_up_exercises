...
describe 'current term page' do
  ...
  context 'when logged in' do
    ...
    it 'displays correct text if term does not exist' do
      expect(page).to have_content('You have not created a budget for this month.')
      expect(page).to have_content('Create one now?')
      expect(page).to have_selector('#create-new-current-term')
    end
