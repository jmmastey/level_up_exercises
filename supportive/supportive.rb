...
class BlagPost
  ...
  def initialize(args)
    ...
    @categories = Array.wrap(args[:categories]) - DISALLOWED_CATEGORIES
  ...
  def category_list
    ...
    "Category".pluralize(categories.length) + ": " +
      categories.map(&:to_s).map(&:titleize).to_sentence
  end

  def commenters
    return '' unless comments_allowed? && comments.any?
    "You will be the #{comments.length.next.ordinalize} commenter"
  end
