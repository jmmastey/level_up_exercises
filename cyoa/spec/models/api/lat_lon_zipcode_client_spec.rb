...
describe LatLonZipcodeClient do
  ...
  #   let(:request) { LatLonZipcodeClient.new("60606 60532") }
  #   it "has the lat_lon_list parameter in the response" do
  #     expect(client.response_parameters).to have_key(:lat_lon_list)
  #   end
  #   it "returns the expected latitude and longetude format" do
  #     expect(client.lat_lon_list).to match(/[\-]?\d{1,2}\.\d+,[\-]?\d{1,3}\.\d+( [\-]?\d{1,2}\.\d+,[\-]?\d{1,3}\.\d+)*/)
  #   end
  #   it "returns the correct latitude and longitude for multiple zipcodes" do
  #     expect(client.lat_lon_list).to eq("41.837,-87.685 41.7918,-88.0878")
  #   end

  #   it "returns the zip codes with corresponding latitude and longetude" do
  #     expect(client.zip_code_lat_lon).to eq({ "60606" => [41.837,-87.685],
  ...
  #   it "should raise" do
  #     expect(LatLonZipcodeClient.new("606 60X32")).to raise_error(LatLonZipcodeError)
  #   end
  # end
end
