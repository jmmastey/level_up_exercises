...
describe Calculator do
  def build_calculator(filename)
    calculator = Calculator.new
    calculator.load(filename)
    calculator
  end

  let(:float_tol) { 1e-8 }

  let(:calculator) { build_calculator('json_test_files/small_test_data.json') }

  let(:stat_calc) { build_calculator('json_test_files/statistics_test.json') }

  let(:conversion_rate_a) { stat_calc.conversion_rate('A') }

  let(:interval_a) { stat_calc.confidence_interval('A', 0.95) }

  it 'should correctly load a small json dataset' do
    expect(calculator.data).to eq([
  ...
  it 'should calculate the conversion rate of visitors' do
    expect(calculator.conversion_rate('A')).to be_within(float_tol).of(0.50)
  end
  ...
  it 'should calculate conversion rate with 95% confidence' do
    expect(conversion_rate_a).to be_between(interval_a[0], interval_a[1])
  end
