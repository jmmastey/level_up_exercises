...
describe GraphJSON, '.to_json' do
  ...
  it 'gives nodes unique id numbers' do
    random_graph = generate_random_graph(max_size)
    random_graph_json = graph_json.to_json(random_graph[0], random_graph[1])
    node_ids = JSON.parse(random_graph_json[0]).map { |node| node['id'] }
